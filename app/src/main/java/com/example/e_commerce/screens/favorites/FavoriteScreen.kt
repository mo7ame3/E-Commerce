...
fun FavoriteScreen(
    ...
    val productData = MutableStateFlow<List<DataX>>(emptyList())

    if (Constant.token.isNotEmpty()) {
        val response: WrapperClass<GetFavorite, Boolean, Exception> =
            produceState<WrapperClass<GetFavorite, Boolean, Exception>>(initialValue = WrapperClass()) {
                value = favoriteViewModel.getFavorite(authorization = Constant.token)
            }.value
        if (response.data?.status == true) {
            scope.launch {
                productData.emit(response.data!!.data.data)
                loading = false
            }
        }
    }
                ...
                if (!productData.value.isNullOrEmpty()) {
                    LazyColumn {
                        items(productData.value) {
                            ImageCardFavAndCart(favItem = it)
